* [面试题总结](#面试题总结)
	* [Android基础](#android基础)
		...
		* [AIDL的理解](#aidl的理解)
	* [框架部分](#框架部分)
		* [RxJava以及原理](#rxjava以及原理)
		* [Retrofit以及原理](#retrofit以及原理)
		* [Volley以及原理](#volley以及原理)
		* [MVP](#mvp)
...
Retrofit对象的创建和Okhttp类似，使用了建造者模式，接口的设计非常的插件化，比如具体的请求类Call，比如转化类Converter，可以方便的支持Gson，比如CallAdapter，可以方便的支持RxJava

在运行过程中首先拿到一个动态代理的Service对象，再拿到具体的Call对象，默认的是OkhttpCall进行网络请求


### Volley以及原理

Volley的最大优点是在网络请求较频繁的时候也可以较快的访问，主要原理是它内部会默认跑1个缓存线程和5个网络请求线程，当我们将网络请求加入Volley的请求队列之后，Volley会先默认从缓存队列中查询，如果缓存队列中不存在才会加入网络请求队友，因为5个网络请求线程的原因，volley适合访问频繁但是数据量小的请求
