...
####  Handle+Message原理
　　弄懂原理需要明白一个重要的东西——Looper，每个线程都会利用ThreadLocal创建一个独有的Looper对象，Looper对象中包含一个独有的MessageQueue，当Looper调用loop方法后会不断循环的从MessageQueue中读取Message进行处理，每个Looper只处理本线程的Message，而Handler对象的作用就是将一个message压入本线程的MessageQueue中，
